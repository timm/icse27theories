#!/usr/bin/env python3
"""Post-process docs/models/*.html to hyperlink paper/sd.py references.

Run after gen_rich.py. Idempotent — only wraps unwrapped <code> nodes.

Sources every `<code>paper/sd.py</code>` and `<code>paper/sd.py:<name></code>`
...
GH_BASE = "https://github.com/timm/icse27theories/blob/main/paper/sd.py"
ROOT    = Path(__file__).resolve().parents[2]
SD_PY   = ROOT / "paper/sd.py"
...
# Match both old (models/sd.py) and new (paper/sd.py) for migration.
UNWRAP = re.compile(
    r'<a class="src-link"[^>]*>\s*(<code>(?:models|paper)/sd\.py(?::\w+)?</code>)\s*</a>')
...
def linkify(html: str) -> str:
    ...
    while True:
        ...
        if new == html:
            break
        html = new
    # Migration: rewrite legacy `models/sd.py` -> `paper/sd.py` in plain code spans
    # so all references are unified before linkification.
    html = re.sub(r"<code>models/sd\.py", "<code>paper/sd.py", html)

    def wrap(m):
        name = m.group(1)
        href = link_for(name)
        if name:
            inner = f"<code>paper/sd.py:{name}</code>"
        else:
            inner = "<code>paper/sd.py</code>"
        return f'<a class="src-link" href="{href}" target="_blank" rel="noopener">{inner}</a>'

    pat = re.compile(r"<code>paper/sd\.py(?::(\w+))?</code>")
